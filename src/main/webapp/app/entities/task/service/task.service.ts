...
export class TaskService {
  ...
  protected convertDateFromClient(task: ITask): ITask {
    return Object.assign({}, task, {
      dataCreate: task.dataCreate?.isValid() ? task.dataCreate.format(DATE_FORMAT) : undefined,
      dataEnd: task.dataEnd?.isValid() ? task.dataEnd.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dataCreate = res.body.dataCreate ? dayjs(res.body.dataCreate) : undefined;
      res.body.dataEnd = res.body.dataEnd ? dayjs(res.body.dataEnd) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((task: ITask) => {
        task.dataCreate = task.dataCreate ? dayjs(task.dataCreate) : undefined;
        task.dataEnd = task.dataEnd ? dayjs(task.dataEnd) : undefined;
      });
    }
    return res;
  }
